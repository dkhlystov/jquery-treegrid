...
(function ($) {
    ...
    $.fn.treegrid.defaults = {
        ...
        iconOpen: 'fa-caret-right', //String to set other Icon in first td when it's close
        iconClose: 'fa-caret-down', //String to set other Icon in first td when it's open
        defaulIconIsNull: false, //Boolean To define if will be display icon (default is true) 
        onActionExpand: null, //Function(id, methods) | the id is of the item clicked, this action will be executed after clicked and if this td not have value (lazy load).
        maxLevelTree: 3, //Number to define limit level in treegrid, after this value the icon open be not available.
        marginLeftAdd: 10, //Number to define default margin left to add each level in treegrid.
        arrayLevelBackgroundColor: null, //Array string to set color background in odd level.
        arrayLevelBackgroundColor2: null, //Array string to set color background in odd par.
        arrayLevelTextColor: null, //Array string to set color text to each level.
        marginExpand: '0.6rem', //String to set margin between icon and label in td.
        clickAllTd: true //Bollean to set if is true user may click all the td, if is false the user may click only in icon.
    };
    ...
    var methods = {
        ...
        addItemInActionExpand: function (idPai, items) {
            var table = document.querySelector(('.treegrid-' + idPai));
            //table.//Encontrar botão e remove-lo.

            var idBase = 2;

            for (var i = 0; i < items.length; i++) {

                idBase = _nextId(idBase);

                items[i] = items[i].replace('<tr>', ('<tr class="treegrid-' + idBase + '">'));
            }

            _add($(table), items);
        },
        getAllParent: function (idPai) {
            var table = $('.treegrid-' + idPai);
            var arrayPai = [];

            for (var i = 0; ; i++) {
                var novoId = _getId(table);
                ...
                table = _getParent(table);
            }

            return arrayPai.reverse();
        },
        addItemInActionExpandWithId: function (idPai, items) {
            var table = document.querySelector(('.treegrid-' + idPai));
    ...
    // PRIVATE FUNCTIONS
    // =================
    function _nextId(idBase) {
        for (; ; idBase++) {
            var ob = document.querySelector(('.treegrid-' + idBase));
    ...
    function _initTree(options) {
        ...
        return $this.each(function () {
            ...
            //event handlers
            if (settings.clickAllTd) {
                $this
                    .on('click', '>tbody>tr>td:first-child>', _expanderClick)
                    .on('mousedown', '>tbody>tr>td:first-child>', _nodeMouseDown);
            }
            else {
                $this
                    .on('click', '>tbody>tr>td:first-child>.treegrid-container>.fa', _expanderClick)
                    .on('mousedown', '>tbody>tr>td:first-child>.treegrid-container', _nodeMouseDown);
            }
            //init nodes
            _initNode(methods.getRoots.call($this), 0);
        });
    };
    function _expand($this) {
        ...
        //expander class
        $this.find('>td:first>.treegrid-container>.fa')
            .removeClass(settings.iconOpen)
            .addClass(settings.iconClose);
    ...
    function _collapse($this) {
        ...
        //expander class
        $this.find('>td:first>.treegrid-container>.fa')
            .removeClass(settings.iconClose)
    ...
    function _initNode($this, depth, forceExpand) {
        ...
        $this.each(function () {
            ...
            $container.find('.fa-caret').remove();

            if (settings.clickAllTd) {
                const typeSpan = '<span class="fa" style="margin-right: ' + settings.marginExpand + '">';
                $expander = $(typeSpan).prependTo($container);
                $container.css('cursor', 'pointer');
            }
            else {
                const typeSpan = '<span class="fa" style="cursor: pointer; margin-right: ' + settings.marginExpand + '">';
                $expander = $(typeSpan).prependTo($container);
            }

            var nivel = numNivel($this);
            ...
            if ($this.data('count')) {
                if ($this.hasClass('expanded')) {
                    $expander.addClass(settings.iconClose);
                    remoreDuplicate($td, settings);
                } else {
                    remoreDuplicate($td, settings);
                    ...
                    if ($child.length > 0) {
                        ...
                        $expander.addClass(settings.iconOpen);
                    }
                }
            }
            else if (!settings.defaulIconIsNull) {
                remoreDuplicate($td, settings);

                if (settings.maxLevelTree > nivel) {
                    if ($child.length > 0) {
                        $expander.addClass(settings.iconClose);
                        $this.addClass('expanded');
                    }
            ...
            if (settings.arrayLevelBackgroundColor != null && $this.css('background').indexOf('rgba(0, 0, 0, 0)') > -1) {
                if (i % 2 == 1) {
                    $this.css('background', settings.arrayLevelBackgroundColor[nivel]);
                }
                else {
                    $this.css('background', settings.arrayLevelBackgroundColor2[nivel]);
                }
                i++;
            }

            if (nivel > 0) {
                $container.css('marginLeft', (nivel * settings.marginLeftAdd));
            }

            if (settings.arrayLevelTextColor != null) {
                $this.css('color', settings.arrayLevelTextColor[nivel]);
            }

            if (_parentCollapsed($this) && comeco) $this.hide();
            _initNode($child, depth + 1, forceExpand);
        });
    };
    function numNivel($this) {
        var qt = 0;
        var result = $this;
        ...
        for (; ;) {
            ...
            var parentID = _getParentId(result);
            if (parentID == null)
                break;

            result = $('.treegrid-' + parentID);
            qt++;
        }
        return qt;
    };
    function remoreDuplicate($td, settings) {
        var icon1 = $td.find('.' + settings.iconClose + ':first')
        var icon2 = $td.find('.' + settings.iconOpen + ':first')

        if (icon1 != null) icon1.remove();
        if (icon2 != null) icon2.remove();
    }
    function _getIdAction(obj) {
        var parent = obj.parent().parent().parent();
        if (parent == null)
            return null;

        var clss = parent.attr('class').split(' ');
        if (clss == null)
            return null;

        const clssFind = 'treegrid-';
        for (var i = 0; i < clss.length; i++) {
            var value = clss[i];

            if (value.indexOf(clssFind) == 0) {
                return value.split(clssFind)[1];
            }
        }
        return null;
    };
    function _buscarThis(obj) {
        var atual = $($(obj).find('.fa'));
    ...
    function _expanderClick() {
        var $this = _buscarThis(this);
        var settings = $this.closest('table').data('treegrid-settings');

        var idclass = _getIdAction($this);

        var tamanhoFilho = $('.treegrid-parent-' + idclass).length;
        if (!settings.defaulIconIsNull && settings.onActionExpand != null && $this.hasClass(settings.iconOpen) && tamanhoFilho == 0) {
            settings.onActionExpand(idclass, methods);

            $this.removeClass(settings.iconOpen);
            $this.addClass(settings.iconClose);

            _expand($this);


        } else if ($this.hasClass(settings.iconOpen) || $this.hasClass(settings.iconClose)) {
    ...
    function _nodeMouseDown(e) {
        ...
        //node expander?
        var $el = $(e.target);
        if ($el.hasClass(iconOpen)) return;
        //handle
    ...
    function _dragStart(e) {
        ...
        $moveHelper = $moveItem.find('>td:first>.treegrid-container').clone().addClass('dragging');

        $moveHelper.find('>.fa').remove();
